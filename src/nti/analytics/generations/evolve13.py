...
def do_evolve():
	...
	if not _column_exists( connection, 'VideoEvents', 'max_time_length' ):
		op.add_column( "VideoEvents", Column( 'max_time_length', Integer, nullable=True ) )

	logger.info( 'Finished analytics evolve13' )

def evolve(context):
	"""
	Evolve to generation 13
	"""
	do_evolve()
