...
class IVideoPlaySpeedChangeEvent(IAnalyticsEvent, IRootContextEvent):
    ...
    OldPlaySpeed = Number(title=u"The old play speed of the video",
                        required=True)

    NewPlaySpeed = Number(title=u"The new play speed of the video",
                          required=True)
...
class IUserProcessedEventsEvent(IObjectEvent):
    """
    Fired after a user events has been processed.
    """
    timestamp = interface.Attribute(u"Timestamp")
    request = interface.Attribute(u"Request")
    events = interface.Attribute(u"Handled Events")


@interface.implementer(IUserProcessedEventsEvent)
class UserProcessedEventsEvent(ObjectEvent):

    def __init__(self, obj, events, request=None, timestamp=None):
        super(UserProcessedEventsEvent, self).__init__(obj)
        self.events = events
